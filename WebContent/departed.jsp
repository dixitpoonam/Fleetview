...
<%!
	...
	String sql,datenew1,datenew2,dt,sdate,dt1,vehlist,location,locationcode,mainuser,locationlist;
			...
			<div class="content-header row">
				<div class="content-header-left col-md-7 col-12 mb-2">
					<div class="row breadcrumbs-top">
						<div class="col-12">
							<h3 class="content-header-title  mb-0">Vehicle Departed Report</h3>
							</div>
							</div>
							</div>
							<div class="content-header-right text-md-end col-md-5 col-12 d-md-block d-none">
				              <div class="mb-1 breadcrumb-right">
					             <div class="demo-inline-spacing mb-1" >
									<button class="engage-help-toggle btn  btn-primary h-35px  btn-color-gray-700 btn-active-color-gray-900 shadow-sm rounded-top-0" data-bs-toggle="modal" data-bs-target="#registerComplaintModal"> Register Complaint</button>
									<button id="kt_engage_demos_toggle" class="engage-demos-toggle btn  btn-primary h-35px  btn-color-gray-700 btn-active-color-gray-900 shadow-sm fs-6rounded-top-0" ><span id="kt_engage_demos_label">Help</span></button>
					 			</div>
							</div>
						</div>
					<h4>Please select the location first.</h4>
					</div>
					</div>
					</div>
	<%}
...
<%
	...
	System.out.println(" buttom :- "+bt);
		...
		if (bt== null || bt.equals("null")) 
		{
			...
			datex1 =  session.getAttribute("onemonthback").toString();  //dd-MMM-yyyy
	...
	<div class="content-wrapper container-xxl p-0">
		<div class="content-header row">
			<div class="content-header-left col-md-7 col-12 mb-2">
				<div class="row breadcrumbs-top">
					<div class="col-12">
						<h3 class="content-header-title  mb-0">Vehicle Departed Report
							<%
								...
								{
									out.print(" from "+session.getAttribute("Location").toString() +" From "+datex1+" to "+datex2 );     
								}
							%>
						</h3>
					</div>
...
<%
			...
			{
				sql="select * from t_completedjourney where GPName='Castrol' and StartDate >='"+dt+"' and StartDate <='"+dt1+"' order by StartDate asc";
			
			}
			else
			{
				sql="select * from t_completedjourney where StartCode in "+locationlist+" and GPName='"+mainuser+"'  and StartDate >='"+dt+"'  and StartDate <='"+dt1+"' order by StartDate asc";
			}
	//out.print(sql);
...
<tr>
	<td align="right"><%=i %></td>
	<td align="right"><a href="startedtrip.jsp?tripid=<%=rst.getString("TripID")%>&vehcode=<%=rst.getString("VehId")%>&sdate=<%=rst.getString("StartDate")%>&vehregno=<%=rst.getString("VehRegNo")%>&ownername=<%=rst.getString("OwnerName")%>&startplace=<%=rst.getString("StartPlace")%>&endplace=<%=rst.getString("endplace")%>&enddate=<%=rst.getString("EndDate")%>&pg=cm"><%=rst.getString("TripID")%> </a></td>
	<td align="left"><%=rst.getString("VehRegNo")%></td>
	<td align="left"><%=rst.getString("OwnerName")%></td>
	<td align="left"><%=sh1%></td>
	<td align="left"><%=sh2%></td>
	<td align="left"><%=rst.getString("StartPlace")%></td>
	<td align="left"><%=rst.getString("EndPlace")%></td>
	<td align="right"><%=rst.getString("KmTravelled")%></td>
	<td align="right">
			<%	
	...
	<td align="right"><%=rst.getString("RunningDuration")%></td>
	<td align="left"><%=rst.getString("DriverName")%></td>
	<td align="right"><%=rst.getString("DriverId")%></td>
</tr>
<%
	i++;
	}	
        ...
        <div class="modal-content">
				...
				<!-- Modal Header -->
					<div class="modal-header">
						<h4 class="modal-title">Custom Column Selection for Vehicle Departed Report </h4>
